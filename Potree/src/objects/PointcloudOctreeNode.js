 ...
 */
PointcloudOctreeNode.memoryThreshold = 500*1000*1000;
//PointcloudOctreeNode.memoryThreshold = 40*1000*1000;
...
PointcloudOctreeNode.prototype.sizeInBytes = function(){
	return this.poc.pointAttributes.bytesPerPoint * this.points;
};
...
PointcloudOctreeNode.loadCloudAjax = function loadPOCCloudAjax(node) {
	if(node.isLoading){
		Logger.error("node is already loading: " + node.name);
		return;
	}
	node.isLoading = true;
